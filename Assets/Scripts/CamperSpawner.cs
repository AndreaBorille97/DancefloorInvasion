...
// EnemySpawner, non va messo su GameManager né su "floor"): ogni tot secondi,
// scelti a caso tra 1 e maxSpawnInterval, istanzia un Camper in un punto casuale
...
{
    [Header("Spawn")]
    [SerializeField] private GameObject camperPrefab; // prefab del Camper da spawnare
    ...
    private void SpawnCamper()
    {
        if (camperPrefab == null || floor == null)
        {
            return;
        }

        Instantiate(camperPrefab, GetRandomPositionOnFloor(), Quaternion.identity);
    }
